  ...
  program = [
    "sh", "-c", <<-EOT
      ...
      if ! command -v zstd >/dev/null 2>&1; then
          printf '\n%s' ' - zstd is not installed or not in PATH. Install it at https://github.com/facebook/zstd/releases' >&2
          missing=1
      fi

      if ! command -v qemu-img >/dev/null 2>&1; then
          printf '\n%s' ' - qemu-img is not installed or not in PATH. Install it via qemu-utils package or https://www.qemu.org/download/' >&2
          missing=1
      fi

      if ! command -v aws >/dev/null 2>&1; then
          printf '\n%s' ' - aws CLI is not installed or not in PATH. Required for cluster state tracking. Install it at https://aws.amazon.com/cli/' >&2
          missing=1
      fi
