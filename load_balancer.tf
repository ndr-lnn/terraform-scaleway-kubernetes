...
locals {
  ingress_service_load_balancer_public_ipv4 = (local.ingress_nginx_service_load_balancer_required && var.ingress_load_balancer_public_network_enabled) ? scaleway_lb_ip.ingress[0].ip_address : null
  ingress_service_load_balancer_public_ipv6 = null # Scaleway LB IPs are IPv4-only
  ingress_service_load_balancer_name        = "${var.cluster_name}-ingress"
}

resource "scaleway_lb_ip" "ingress" {
  count   = (local.ingress_nginx_service_load_balancer_required && var.ingress_load_balancer_public_network_enabled) ? 1 : 0
  zone    = var.scaleway_zone
...
resource "scaleway_lb" "ingress" {
  ...
  ip_ids = var.ingress_load_balancer_public_network_enabled ? [scaleway_lb_ip.ingress[0].id] : []
  zone   = var.scaleway_zone
...
resource "scaleway_lb_backend" "ingress_http" {
  ...
  forward_port_algorithm = var.ingress_load_balancer_algorithm
...
resource "scaleway_lb_frontend" "ingress_http" {
  ...
  lb_id        = scaleway_lb.ingress[0].id
  backend_id   = scaleway_lb_backend.ingress_http[0].id
  name         = "ingress-http"
  inbound_port = 80
}
...
resource "scaleway_lb_frontend" "ingress_https" {
  ...
  lb_id        = scaleway_lb.ingress[0].id
  backend_id   = scaleway_lb_backend.ingress_https[0].id
  name         = "ingress-https"
  inbound_port = 443
}
